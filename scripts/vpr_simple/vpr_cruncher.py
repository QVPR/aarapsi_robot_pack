...
from pyaarapsi.vpr_simple.imageprocessor_helpers import Tolerance_Mode, FeatureType, ViewMode
...
from pyaarapsi.core.argparse_tools  import check_bounded_float, check_positive_float, check_positive_two_int_tuple, check_positive_int, \
                                           check_bool, check_enum, check_string
from pyaarapsi.core.ros_tools       import q_from_yaw, roslogger, get_ROS_message_types_dict, set_rospy_log_lvl, LogType, NodeState, ROS_Home
from pyaarapsi.core.helper_tools    import formatException

class mrc: # main ROS class
    def __init__(self, compress_in, compress_out, do_groundtruth, rate_num, namespace, node_name, anon, log_level, reset):
    ...
    def init_params(self, rate_num, log_level, compress_in, compress_out, do_groundtruth, reset):
        ...
        self.REF_DATA_NAME          = self.ROS_HOME.params.add(self.NAMESPACE + "/ref/data_name",       None,                   check_string,                                                   force=False)
        self.REF_IMG_PATH           = self.ROS_HOME.params.add(self.NAMESPACE + "/ref/images_path",     None,                   check_string,                                                   force=False)
        self.REF_ODOM_PATH          = self.ROS_HOME.params.add(self.NAMESPACE + "/ref/odometry_path",   None,                   check_string,                                                   force=False)
        self.REF_VIEW               = self.ROS_HOME.params.add(self.NAMESPACE + "/ref/view",            None,                   lambda x: check_enum(x, ViewMode),                              force=False)
    ...
    def init_vars(self):
        ...
        self.rolling_mtrx_img       = np.zeros((len(self.ref_dict['odom']['position']['x']),)*2) # Make empty similarity matrix figure
    ...
    # top matching reference index for query
        ...
        dvc = fastdist.matrix_to_matrix_distance(self.ref_dict['img_feats'][enum_name(self.FEAT_TYPE.get())][enum_name(self.REF_VIEW.get())], \
                                                 np.matrix(ft_qry),\
    ...
    def publish_ros_info(self, cv2_img, tInd, mInd, dvc, mPath, state):
    ...
    def loop_contents(self):
        ...
        ft_ref          = self.ref_dict['img_feats'][enum_name(self.FEAT_TYPE.get())][enum_name(self.REF_VIEW.get())][matchInd]
        ...
        # Make ROS messages
        self.publish_ros_info(img_to_pub, trueInd, matchInd, dvc, str(self.ref_dict['image_paths']).replace('\'',''), tolState)
