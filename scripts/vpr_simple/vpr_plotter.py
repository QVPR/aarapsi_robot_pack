...
from pyaarapsi.core.ros_tools import roslogger, get_ROS_message_types_dict, set_rospy_log_lvl, NodeState, LogType, ROS_Home
...
class mrc: # main ROS class
    def __init__(self, compress_in, rate_num, namespace, node_name, anon, log_level, reset):
...
class Doc_Frame:
    def __init__(self, nmrc, printer=print):
        ...
        self.rolling_mtrx_img = np.zeros((len(nmrc.ref_dict['odom']['position']['x']), len(nmrc.ref_dict['odom']['position']['x']))) # Make similarity matrix figure
        ...
        self.fig_xywv_handles = doXYWVFigBokeh(nmrc.ref_dict['odom']) # Make linear&angular metrics figure
        self.fig_dvec_handles = doDVecFigBokeh(nmrc.ref_dict['odom']) # Make distance vector figure
        self.fig_fdvc_handles = doFDVCFigBokeh(nmrc.ref_dict['odom']) # Make filtered dvc figure
        self.fig_odom_handles = doOdomFigBokeh(nmrc.ref_dict['odom']) # Make odometry figure
        self.fig_svmm_handles = doSVMMFigBokeh(nmrc.ref_dict['odom']) # Make SVM metrics figure
...
# Main loop process
    ...
    updateXYWVFigBokeh(doc_frame, matchInd, nmrc.ref_dict['odom'])
    updateDVecFigBokeh(doc_frame, matchInd, trueInd, dvc, nmrc.ref_dict['odom'])
    updateFDVCFigBokeh(doc_frame, matchInd, trueInd, dvc, nmrc.ref_dict['odom'])
    updateOdomFigBokeh(doc_frame, matchInd, trueInd, nmrc.ref_dict['odom'])
    ...
    updateSVMMFigBokeh(doc_frame, nmrc.state, nmrc.ref_dict['odom'])
